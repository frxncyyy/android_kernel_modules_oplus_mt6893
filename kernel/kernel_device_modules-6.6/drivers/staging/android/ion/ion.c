...
/*
 * plist_add() is not exported to modules in 6.6.  ION's heap list is priority
 * ordered ("attempt higher id numbers first"), and ion_ioctl() picks a heap by
 * walking that order, so keep the exact plist semantics with a local copy of the
 * upstream insertion.  Only plist_add is missing; the rest of <linux/plist.h> is
 * inline or macro.
 */
...
void ion_device_add_heap(struct ion_heap *heap)
{
	...
	if (!debug_file)
		pr_err("Failed to create heap debugfs for %s\n", heap->name);
...
MODULE_VERSION("1.0");
/* dma_buf_export/fd/put are in the DMA_BUF namespace in 6.6; a module must import
 * the namespace explicitly or modpost refuses to resolve them. */
