...
static unsigned int __ion_ptr_to_hash(const void *ptr)
{
	...
	/* ptr_to_hashval() is not exported to modules in 6.6, so hash locally.
	 * The value is only a tracepoint correlation id, not an address. */
	hashval = ((unsigned long)ptr) >> 4;
