...
operation and HBM still need validation. The vendor always-on display works on
the port as of round 39: the panel driver registers the `doze_get_mode_flags`
hook the display driver requires before it calls `doze_post_disp_on` (the `0x29`
Display On that brings the panel back), returning the command-mode flags this
port runs with - the stock video-mode values corrupted the panel on the AOD
exit. See
