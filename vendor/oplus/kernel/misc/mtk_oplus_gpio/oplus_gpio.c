...
static irqreturn_t sim_det_irq_thread(int irq, void *dev_id)
{
	struct sim_det_data *cd = (struct sim_det_data *)dev_id;
	cd->gpio_status = 1;
	OPLUS_GPIO_MSG("sim_det_irq_thread irq:%d", cd->sim_det_irq);
	return IRQ_HANDLED;
}

int sim_det_setup_irq(struct sim_det_data *cd)
{
	int rc, irq;
	unsigned long irq_flag_type;

	OPLUS_GPIO_MSG("enter sim_det_setup_irq\n");
	if (cd == NULL) {
		pr_err("%s: sim_det_data is null\n", __func__);
		return -EINVAL;
	}
	OPLUS_GPIO_MSG("enter sim_det_setup_irq gpio=%d\n", cd->gpio);

	irq = gpio_to_irq(cd->gpio);
	irq_flag_type = IRQF_TRIGGER_RISING | IRQF_ONESHOT | IRQF_NO_SUSPEND;
	rc = request_threaded_irq(irq, NULL, sim_det_irq_thread, irq_flag_type, "sim2_det", cd);
	if (rc) {
		pr_err("%s: sim_det_setup_irq irq:%d,rc:%d", __func__, irq, rc);
		return rc;
	}
	OPLUS_GPIO_MSG("sim_det_setup_irq irq:%d,rc:%d", irq, rc);
	cd->sim_det_irq = irq;
