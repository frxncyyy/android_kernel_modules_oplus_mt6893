...
/* Plain names: drivers/gpu/drm/oplus is a symlink into the vendor tree, so the
 * stock "../mediatek/mediatek_v2/..." spelling cannot resolve.  panel/Makefile
 * puts drivers/gpu/drm/mediatek/mediatek_v2 on the include path instead.
 */
...
#include "mtk_corner_pattern/oplus20615_all_mtk_data_hw_roundedpattern.h"
#include "mtk_corner_pattern/oplus20615_all_mtk_data_hw_roundedpattern_l.h"
#include "mtk_corner_pattern/oplus20615_all_mtk_data_hw_roundedpattern_r.h"
#endif
...
/*
 * op6893 6.6 port: in the 4.19 tree the block below was guarded by
 * OPLUS_BUG_STABILITY and every symbol was an extern provided by Oplus' 4.19
 * mtk_drm/AAL code.  The 6.6 module drop reworked that code; three of them
 * (oplus_dc_alpha, oplus_display_brightness, oplus_max_normal_brightness) do
 * survive as exported globals of mediatek-drm, BUT importing them makes this
 * panel depend on mediatek-drm.ko, which forces the panel to load *after* it.
 * That is fatal: mtk_dsi_probe() tears the mipi_dsi child devices back down
 * (mipi_dsi_host_unregister on its EPROBE_DEFER path) when no panel driver is
 * registered yet, so a panel that only appears after mediatek-drm never binds.
 * The stock ROM loads panels BEFORE mediatek-drm.  To allow that we keep ALL of
 * these panel-local (no cross-module import), so this .ko has zero mediatek-drm
 * dependency.  The only cost is that framework-driven DC-dimming hand-off is
 * inert during bring-up, which does not affect basic init / enable / backlight.
 */
#define RAMLESS_AOD_PAYLOAD_SIZE	100
#define OPLUS_DC_BACKLIGHT_THRESHOLD 650
int oplus_dc_alpha;
unsigned long oplus_display_brightness;
unsigned long oplus_max_normal_brightness;

u32 flag_writ;
u32 flag_hbm;
int oplus_dc_enable_real;
int oplus_dc_enable;
int exit_dc_flag;
...
/*
 * 6.6 changed this to disp_aal_set_dre_en(struct mtk_ddp_comp *, int) and does
 * not export it, and a panel driver has no mtk_ddp_comp to pass.  DRE toggling
 * around AOD entry/exit is a picture-quality refinement, not required to bring
 * the panel up, so stub it out.
 */
static inline void disp_aal_set_dre_en(int enable) { }
...
struct LCM_setting_table {
	unsigned int cmd;
	unsigned char count;
	/* 130, not the 4.19 value of 128: the DSC setup entry in
	 * lcm_normal_to_aod_sam[] is {REGFLAG_CMD, 129, {0x9E, ...}} i.e. 129
	 * bytes, which overflowed the array.  4.19 only warned about it.
	 */
...
/* Dead in the 4.19 source too: its ext_funcs entry is commented out there.
 * Kept verbatim (and behaviour-identical to 4.19) but marked __maybe_unused
 * so the 6.6 build's -Werror=unused-{variable,function} does not trip.
 */
...
/*
 * 4.19 kept this in mtk_dsi.c ("int power_mode = 2") and updated it from the
 * doze paths: 2 = normal on, 1 = AOD on, 3 = AOD off, 0 = normal off.  6.6's
 * mtk_dsi.c has no equivalent, so define it locally at the 4.19 initial value.
 * The only user is the lcm_prepare() gate below, and 2 is exactly the cold-boot
 * value, so lcm_panel_init() runs as it does on 4.19.  AOD entry/exit in 6.6 is
 * driven through the doze_* callbacks, which do not go through lcm_prepare().
 */
int power_mode = 2;
...
/* 6.6 dropped drm_display_mode.vrefresh; the refresh rate is derived with
 * drm_mode_vrefresh() = DIV_ROUND_CLOSEST(clock * 1000, htotal * vtotal), and
 * mediatek_v2 (get_mode_enum() style code) relies on that giving the nominal
 * rate.  htotal * vtotal = 1150 * 2430 = 2794500, so the 4.19 .clock values
 * (163530 -> 59 Hz, 327060 -> 117 Hz) had to be corrected to 60/120 Hz.
 * The DSI data rate is unaffected: it comes from ext_params.data_rate.
 */
...
static struct mtk_panel_params ext_params = {
	...
	.hbm_en_time = 2,
	.hbm_dis_time = 2,
	/* 6.6 mtk_panel_params has no oplus_hbm_on_sync_with_flush /
	 * oplus_disable_hdr_d65 / dsc_output_fhd / oplus_bypass_color_flag;
	 * they were HDR/DC colour tweaks, dropped for the 6.6 port.
	 */
...
/* 6.6 added a struct drm_connector * argument.  @mode is still the same
 * connector mode index that 4.19 passed (mtk_dsi.c hands us dst_mode_idx /
 * CRTC_PROP_DISP_MODE_IDX), so the 0=60Hz / 1=120Hz mapping is unchanged.
 */
...
/* 6.6 dropped mtk_panel_funcs.lcm_dc_post_exitd, so this is no longer reachable
 * from mediatek-drm.  Kept (unused) so the DC-dimming exit sequence is not lost
 * if a 6.6 hook for it turns up.
 */
static int __maybe_unused oplus_lcm_dc_post_exitd(void *dsi, dcs_write_gce cb,void *handle)
{
	int i;
...
/* 6.6 removed mtk_panel_funcs.hbm_set_state (only the getter is left); the
 * state is now maintained panel-side, and panel_hbm_set_cmdq() below already
 * does "ctx->hbm_en = en", so the old setter callback is redundant.
 */
static void __maybe_unused panel_hbm_set_state(struct drm_panel *panel, bool state)
{
	struct lcm *ctx = panel_to_lcm(panel);
...
static struct mtk_panel_funcs ext_funcs = {
	...
	/* .hbm_set_state: removed from mtk_panel_funcs in 6.6 */
	...
	/* .lcm_dc_post_exitd: removed from mtk_panel_funcs in 6.6 */
};
#endif
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (remote_node != dev->of_node) {
		pr_info("skip probe due to not current lcm,dev->of_node is %pOF\n", dev->of_node);
		return 0;
	}
...
/* 6.6: mipi_dsi_driver.remove returns void */
...
/* 4.19 built this panel into the kernel, so it carried no MODULE_* trailer;
 * as a 6.6 loadable module modpost requires at least MODULE_LICENSE().
 */
MODULE_AUTHOR("OPLUS Display");
MODULE_DESCRIPTION("Samsung AMS643YE01 oplus20615 pvt 1080p DSI CMD Panel Driver");
